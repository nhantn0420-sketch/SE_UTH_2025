...
See detailed authentication sequence diagram: [Figure 4.3.7: Authentication Flow](#1-user-authentication-flow)
...
![Figure 4.3.1: User & Authentication Module Class Diagram](../Images/User&AuthenticationModule-ClassDiagram.png)
...
![Figure 4.3.2: Academic Module Class Diagram](../Images/AcademicModule-ClassDiagram.png)
...
![Figure 4.3.3: Project Module Class Diagram](../Images/ProjectModule-ClassDiagram.png)
...
![Figure 4.3.4: Group Module Class Diagram](../Images/GroupModule-ClassDiagram.png)
...
![Figure 4.3.5: Collaboration Module Class Diagram](../Images/CollaborationModule-ClassDiagram.png)
...
![Figure 4.3.6: Evaluation Module Class Diagram](../Images/EvaluationModule-ClassDiagram.png)
...
![Figure 4.3.7: Authentication Flow](../Images/UserAuthenticationFlow.png)
...
![Figure 4.3.9: Approve Project Flow](../Images/ApproveProjectFlow.png)
...
![Figure 4.3.10: Create Team Flow](../Images/CreateTeamandAddMembersFlow.png)
...
![Figure 4.3.12: Submit Checkpoint](../Images/SubmitCheckpointwithFile.png)
...
![Figure 4.3.15: Real-time Chat](../Images/Real-time ChatwithWebSocket.png)
...
![Figure 4.3.16: Video Call Flow](../Images/VideoCallwithWebRTC.png)
...
This section provides quick references to the detailed sequence diagrams presented in section 4.3.6. For complete interaction flows with full technical details, refer to the corresponding figures:

### 1. User Authentication Flow

**Summary**: User login with JWT token generation and credential validation.

**See detailed diagram**: [Figure 4.3.7: Authentication Flow](#1-user-authentication-flow)

**Key steps**: User enters credentials → Backend validates with bcrypt → JWT tokens generated → Tokens stored in localStorage → User redirected to dashboard.

---

### 2. Project Approval Workflow

**Summary**: Lecturer creates project, submits for approval, Department Head reviews and approves/denies.

**See detailed diagrams**: 
- Project creation: [Figure 4.3.8: Create Project Flow](#2-create-project-with-ai-milestone-generation)
- Approval process: [Figure 4.3.9: Approve Project Flow](#3-approve-project-flow)

**Key steps**: Lecturer creates project (status: PENDING) → Lecturer submits for review → Head receives notification → Head approves or denies with reason → Lecturer receives notification → Approved projects available for team selection.

---

### 3. Real-time Chat Sequence

**Summary**: Bidirectional real-time chat using WebSocket (Socket.IO) for team communication.

**See detailed diagram**: [Figure 4.3.15: Real-time Chat](#9-real-time-chat-with-websocket)

**Key steps**: User connects to Socket.IO → Joins team room → Types message → Message saved to database → Broadcast to all room members → All clients receive instantly (<500ms latency).

---

### 4. Peer Review Submission

**Summary**: Students anonymously evaluate team members using 4-dimensional scoring system.

**See detailed diagram**: [Figure 4.3.14: Peer Review Flow](#8-peer-review-process)

**Key steps**: Student fills review form (cooperation, contribution, communication, technical scores) → Backend validates same-group constraint → Reviews saved as anonymous → Aggregated scores calculated → Lecturer receives notification.

---

### 5. AI Milestone Generation

**Summary**: AWS Bedrock Claude generates project milestones automatically from project description.

**See detailed diagram**: [Figure 4.3.8: Create Project Flow](#2-create-project-with-ai-milestone-generation) (includes AI milestone generation)

**Key steps**: Lecturer enters project details → Clicks "Generate Milestones with AI" → Backend sends prompt to AWS Bedrock → AI streams milestone suggestions → Lecturer reviews/edits → Lecturer approves → Milestones batch inserted to database.
