...
![Figure 4.1: System Architecture Diagram](../Images/SystemArchitecture.png)
...
![Figure 4.3: Authentication Flow Sequence](../Images/UserAuthenticationFlow.png)
...
*Figure 4.4: **RBAC Hierarchy and Permission Mapping** - Complete role-based access control system showing hierarchical role inheritance (Admin > Staff > Head of Department > Lecturer > Student) and fine-grained permission mapping across 6 modules (User, Academic, Project, Group, Collaboration, Evaluation). Each role has specific permissions for CRUD operations, with Admin having full system access, Lecturers managing their classes and projects, and Students limited to team collaboration and submission features. Authorization implemented via JWT tokens with role claims validated by middleware on each API endpoint.*
...
1. **Admin (Super Admin)** - Platform Owner
   ...
   - System configuration: Manage departments, system settings
   - Access to all modules and audit logs

2. **Staff** - Administrative Support
   - Import subjects, classes, and users (bulk operations)
   - Manage academic structure (departments, curricula)
   - View system-wide reports and analytics
   - Cannot modify projects or evaluate students

3. **Head of Department** - Academic Leadership
   - Approve/reject project proposals from lecturers
   - Assign approved projects to specific classes
   - View all lecturer activities within department
   - Generate department-level performance reports
   - Cannot directly create projects (only approve)

4. **Lecturer** - Course Instructor
   - Create and manage projects with milestones
   - Create and manage student teams within classes
   - Evaluate team checkpoints and final submissions
   - View assigned classes and student progress
   - Real-time chat and video calls with students
   - Cannot access other lecturers' classes

5. **Student** - Team Member (Lowest Privilege)
   - View assigned classes and team information
   - Manage tasks in personal workspace
   - Submit checkpoints with file uploads
   - Participate in peer reviews (anonymous)
   - Collaborate via chat and video calls
   - Cannot access evaluation criteria or other teams' work

**Permission Mapping by Module:**

| Module | Admin | Staff | Head of Dept | Lecturer | Student |
|--------|-------|-------|--------------|----------|----------|
| **User Management** | Full CRUD | Read only | Read department | Read class | Read self |
| **Academic (Subject/Class)** | Full CRUD | Import & CRUD | Read department | Read assigned | Read enrolled |
| **Project Management** | Full CRUD | Read only | Approve/Reject | Create/Manage | Read/Pick |
| **Group Management** | Full CRUD | Read only | Read department | Create/Manage | Manage own team |
| **Collaboration (Chat/Call)** | Monitor all | No access | No access | Access class chats | Access team chats |
| **Evaluation** | View all | View reports | View department | Evaluate & grade | Submit & view own |

**Key RBAC Features:**
- ✅ Hierarchical role inheritance (higher roles inherit lower role permissions)
- ✅ Resource-based authorization (users can only modify resources they own)
- ✅ JWT-based authentication with role claims embedded in token payload
- ✅ Middleware validation on every protected API endpoint
- ✅ Fine-grained permissions per HTTP method (GET, POST, PUT, DELETE)
- ✅ Audit logging for all sensitive operations (project approval, grade changes)
- ✅ Role assignment restricted to Admin users only

**Authorization Flow:**
```
1. User logs in → Backend generates JWT with role claim
2. Frontend stores token in localStorage
3. Each API request includes: Authorization: Bearer <token>
4. Middleware decodes JWT and extracts user_id + role
5. Route handler checks if role has permission for requested action
6. If authorized → Execute business logic
7. If denied → Return 403 Forbidden
```

**Example Permission Checks:**
```python
# Lecturer can only evaluate teams in their own classes
...
async def create_evaluation(
    ...
    if current_user.role not in ["lecturer", "admin"]:
        raise HTTPException(403, "Only lecturers can evaluate")
    
    # Check lecturer owns the class
    class_obj = await db.get(Class, evaluation_data.class_id)
    if class_obj.lecturer_id != current_user.id and current_user.role != "admin":
