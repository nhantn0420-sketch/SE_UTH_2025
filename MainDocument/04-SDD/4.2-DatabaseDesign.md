...
![Figure 4.2.2: Conceptual Database Model](../Images/Conceptual Model Project-Based Learning Management System.png)

*Figure 4.2.2: Mô hình Conceptual Database của CollabSphere - Hiển thị kiến trúc cấp cao với 6 nhóm thực thể chính: USERS (Admin, Staff, Head, Lecturer, Student), ACADEMIC (Subject, Class, Members), PROJECTS (Project, Milestone), GROUPS & TEAMS (Group, Group Members, Tasks & Checkpoints), COLLABORATION (Chat, Meetings, Resources, Whiteboard), EVALUATION (Peer Review, Evaluations, Feedback), và NOTIFICATION (Email, Real-time). Các mũi tên thể hiện luồng dữ liệu và mối quan hệ giữa các nhóm thực thể trong hệ thống quản lý Project-Based Learning.*
...
![Figure 4.2: Entity Relationship Diagram](../Images/EntityRelationshipDiagram.png)
...
**Diagram Legend:**
- 🔑 **PRIMARY_KEY**: Gold indicator for primary key fields
- 🔗 **FOREIGN_KEY**: Blue indicator for foreign key relationships
- ⚡ **UNIQUE**: Green indicator for unique constraints
...
![Figure 4.2.3: Users & Authentication Module](../Images/Module1_Users&Authentication.png)

*Figure 4.2.3: Module Users & Authentication - Bảng users là bảng trung tâm quản lý tất cả người dùng trong hệ thống với RBAC (Role-Based Access Control). Bao gồm 5 vai trò: ADMIN, STAFF, HEAD, LECTURER, STUDENT. Password được mã hóa bằng bcrypt, email dùng làm authentication. Bảng có các unique constraints trên username và email, cùng với các indexes để tối ưu truy vấn. Users có relationships 1:N với nhiều bảng khác: class_members (as student), classes (as lecturer), group_members (as member), projects (as creator), chat_messages (as sender), evaluations (as evaluator), và notifications (as recipient).*

### Module 2: Academic Management

![Figure 4.2.4: Academic Management Module](../Images/Module2_AcademicManagement.png)

*Figure 4.2.4: Module Academic Management - Gồm 4 bảng quản lý thông tin học thuật: (1) **subjects** - Danh mục môn học với mã môn (code) duy nhất và số tín chỉ; (2) **curricula** - Nội dung giảng dạy theo tuần cho mỗi môn học; (3) **classes** - Lớp học cụ thể trong học kỳ, được phụ trách bởi Lecturer, có giới hạn sinh viên và trạng thái (ACTIVE/INACTIVE/COMPLETED); (4) **class_members** - Quan hệ many-to-many giữa students và classes, với role (MEMBER/MONITOR). Relationships: Subject → Classes (1:N), Subject → Curricula (1:N), Class → Members (1:N), Lecturer → Classes (1:N), Student → Class Members (1:N).*

### Module 3: Project & Group

![Figure 4.2.5: Project & Group Management Module](../Images/Module3_Project&GroupManagement.png)

*Figure 4.2.5: Module Project & Group Management - Module phức tạp nhất với 9 bảng: (1) **projects** - Đề tài dự án template do Lecturer tạo và Head phê duyệt, có trạng thái PENDING/SUBMITTED/APPROVED/REJECTED; (2) **project_milestones** - Các cột mốc của project template; (3) **class_projects** - Gán project cho classes; (4) **groups** - Nhóm sinh viên làm dự án, mỗi nhóm có 1 leader và 1 project; (5) **group_members** - Thành viên trong nhóm với contribution_score; (6) **group_milestones** - Milestone cụ thể của nhóm với deadline; (7) **checkpoints** - Điểm kiểm tra/nộp bài; (8) **tasks** - Kanban board tasks với status TODO/IN_PROGRESS/DONE. Workflow: Lecturer creates Project → Head approves → Assigned to Classes → Students form Groups → Groups implement Projects → Track Milestones/Tasks.*

### Module 4: Collaboration

![Figure 4.2.6: Collaboration Tools Module](../Images/CollaborationTools.png)

*Figure 4.2.6: Module Collaboration Tools - Gồm 7 bảng hỗ trợ làm việc nhóm: (1) **meetings** - Cuộc họp nhóm với Zoom/Google Meet integration, status SCHEDULED/IN_PROGRESS/COMPLETED/CANCELLED; (2) **meeting_participants** - Người tham gia meeting với trạng thái INVITED/ACCEPTED/DECLINED/ATTENDED; (3) **chat_messages** - Chat real-time cho group hoặc class, hỗ trợ TEXT/FILE/IMAGE/VIDEO với read receipts; (4) **resources** - File management với Cloudinary integration, hỗ trợ PDF/DOCX/XLSX/PPT, có thể share ở group hoặc class level; (5) **whiteboard_sessions** - Bảng trắng cộng tác real-time lưu dữ liệu JSON; (6) **document_sessions** - Chỉnh sửa tài liệu cộng tác (HTML/Markdown) giống Google Docs. Integration: Socket.IO (real-time), Cloudinary (storage), Zoom/Google Meet (video).*

### Module 5: Evaluation

![Figure 4.2.7: Evaluation & Assessment Module](../Images/Evaluation&Assessment.png)

*Figure 4.2.7: Module Evaluation & Assessment - Gồm 5 bảng đánh giá đa chiều: (1) **peer_reviews** - Đánh giá đồng nghiệp Student→Student với rating 1-5 sao, có thể ẩn danh, theo period (Midterm/Final/Weekly), criteria JSON tùy chỉnh (communication, contribution, collaboration); (2) **group_evaluations** - Lecturer đánh giá nhóm với score 0-10, evaluation_type: Progress/Final/Presentation; (3) **member_evaluations** - Lecturer đánh giá từng thành viên dựa trên contribution_score và peer reviews; (4) **checkpoint_evaluations** - Đánh giá các deliverables cụ thể với quick feedback; (5) **milestone_answers** - Nhóm trả lời câu hỏi milestone để track progress. Score scales: Peer reviews 1-5 stars, tất cả khác 0-10 points.*
